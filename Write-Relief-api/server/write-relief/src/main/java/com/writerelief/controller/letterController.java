...
@RequestMapping("/letters") // Specifies that the URL for all the methods in this controller will start with
                            // "/letters"
...
// Endpoint for creating a new letter. It handles POST requests to "/letters"
  ...
  public Mono<ResponseEntity<Letter>> createLetter(@RequestBody Mono<Letter> letterMono) {
    ...
    // Calls saveLetter method from letterService with the Mono<Letter> from the request body
...
// Endpoint for deleting a letter by its ID. It handles DELETE requests to "/letters/{id}"
  @DeleteMapping("/{id}")
...
// Endpoint for retrieving all letters for a specific user. It handles GET requests to "/letters/user/{userId}"
  @GetMapping("/user/{userId}")
  public Flux<Letter> getLettersByUserId(@PathVariable String userId) {
    // Calls findByUserId method from letterService to fetch letters belonging to a specific user
